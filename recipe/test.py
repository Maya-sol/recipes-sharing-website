...
from recipefl import app, db, Recipe, User
...
def test_wrong_password(client):
    register(client)
    rv = client.post('/login', data={'username': 'alice', 'password': 'wrong'}, follow_redirects=True)
    assert b'Login' in rv.data
...
def test_view_recipe(client):
    register(client)
    login(client)
    client.post('/add', data={'title': 'Soup', 'ingredients': 'Water', 'instructions': 'Boil'}, follow_redirects=True)
